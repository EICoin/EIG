...
unsigned int nStakeMinAge = 1 * 30 * 60; // 24 hours
unsigned int nStakeMaxAge = -1;           // 30 days
...
const string strMessageMagic = "EI Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if (pindexBest->nHeight+1 <= 522800)
    {
      nSubsidy = 0.1 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 10430000)
    {
      nSubsidy = 0.1 * COIN;
      return nSubsidy + nFees;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "EI", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "happy life";
        CTransaction txNew;
        txNew.nTime = 1473530030;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xaca735e7a095676a94f2d9b926fd5465c0aa7545c3ccedd56aba7636b9ade0f0"));
        block.print();
